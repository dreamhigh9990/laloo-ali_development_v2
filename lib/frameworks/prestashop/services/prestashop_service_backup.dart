...
class PrestashopService extends BaseServices {
  ...
  Future<Order> createOrder(
    ...
    try {
      ...
      var params =
          'products=${productsId.toString()}&quantity=${productsQuantity.toString()}&attribute="1"';
      params +=
          '&id_carrier=$idCarrier&id_lang=$idLang&id_customer=$idCustomer';
      params +=
          '&id_currency=$idCurrency&id_address_delivery=$idAddressDelivery';
      params +=
          '&id_address_invoice=$idAddressInvoice&current_state=$currentState';
      params +=
          '&payment=$payment&module=$module&total_shipping=$totalShipping&total_products=$totalProducts&display=full';
  ...
  Future<String> createAddress(CartModel cartModel, UserModel? user) async {
    try {
      var param = '';
      param += 'id_customer=${user?.user?.id ?? 'guest'}';
