...
class _HomeLayoutState extends State<HomeLayout> with AppBarMixin {
  ...
  loadedLanding() async {
    try {
      if (landingData.isEmpty) {
        const String menues =
            'https://X2SHFNYYQKHYMZ46Q8KF3J4K3W5449WT@laloo.gr/api/landing/1';
        ...
        List<dynamic> landing =
            data['prestashop']['landing']['data']['language'];
        for (final itme in landing) {
          if (itme['_id'] == langId && mounted) {
            setState(() {
              landingData = jsonDecode(itme['value']);
            });
          }
        }
      }
    } catch (e) {
      logger.e('loadedLanding err', e);
    }
  }
            ...
            SliverList(
              delegate: SliverChildBuilderDelegate(
                (context, index) {
                  ...
                  if (landingData.isNotEmpty) {
                    return PreviewOverlay(
                      index: previewIndex,
                      config: config,
                      builder: (value) {
                        return DynamicLayout(
                            landingDatas: landingData,
                            config: value,
