...
class PaymentMethodItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    var updatedTitle;
    if (paymentMethod.id!.contains('ps_cashondelivery')) {
      if (lang == 'el') {
        updatedTitle = 'Αντικαταβολή';
      } else {
        updatedTitle = 'Pay by Cash on Delivery';
      }
    } else if (paymentMethod.title!.toString().contains('Vivapay')) {
      updatedTitle = 'Viva wallet';
    } else {
      updatedTitle = paymentMethod.title;
    }
    return Column(
      children: <Widget>[
        InkWell(
